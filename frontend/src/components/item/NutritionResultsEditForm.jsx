 ...
 *
 * Stage 8 edit-mode renderer for nutrition results. Flips all editable
 * nutrition fields into controlled inputs: healthiness score + rationale,
 * five macros, and a micronutrient chip list (add/remove).
 *
 * Parent (NutritionResults) owns the view/edit state and the server
 * round-trip via `onSave(payload)` / `onCancel()`. `saving` disables
 * Save while the POST is in flight.
 */
const NutritionResultsEditForm = ({
  initialValues,
  onSave,
  onCancel,
  saving,
  ...
  return (
    <div
      ...
      data-testid="nutrition-edit-form"
    >
      <div className="border-b pb-4 flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button
            ...
            data-testid="nutrition-edit-cancel"
          >
            Cancel
          </button>
          <button
            ...
            data-testid="nutrition-edit-save"
          >
            {saving ? "Saving..." : "Save"}
          </button>
        </div>
      ...
      <div className="space-y-3">
        <label className="block text-sm font-semibold text-gray-700">
          ...
          <input
            ...
            data-testid="nutrition-healthiness-score-input"
          />
        </label>
        <label className="block text-sm font-semibold text-gray-700">
          Rationale
          <textarea
            ...
            rows={4}
            className="mt-1 block w-full rounded border-gray-300 shadow-sm"
            data-testid="nutrition-rationale-input"
          />
        </label>
      </div>
        ...
        <div className="flex flex-wrap gap-2">
          {micros.map((name, idx) => (
            <span
              ...
              data-testid={`nutrition-micro-chip-${name}`}
            >
              {name}
              <button
                type="button"
...
export default NutritionResultsEditForm;
