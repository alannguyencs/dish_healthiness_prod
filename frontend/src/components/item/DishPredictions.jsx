 ...
 */
const DishPredictions = ({ predictions = [], selectedDish, onDishSelect, disabled = false }) => {
  ...
  if (!predictions || predictions.length === 0) {
    return (
      <div className="mb-4">
        ...
        />
        <p className="mt-1 text-sm text-gray-500">No dish predictions available</p>
      </div>
    );
  }
  ...
  const isCustom = selectedDish && !predictions.find(p => p.name === selectedDish);
    ...
    <div className="mb-4 relative" ref={dropdownRef}>
      ...
      >
        <div className="flex items-center justify-between">
          <div className="flex-1 min-w-0">
            <div className="font-medium text-gray-900 truncate">{selectedDish || 'Select a dish'}</div>
            {selectedPrediction && (
      ...
      {!isTopChoice && selectedDish && (
        <p className="mt-1.5 text-xs text-gray-600">
          AI suggested: <span className="font-medium">{topPrediction.name}</span> ({Math.round(topPrediction.confidence * 100)}%)
        </p>
      )}
                ...
                <div className="flex items-center justify-between gap-2">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-sm font-medium text-gray-900 truncate">{prediction.name}</span>
                      {index === 0 && (
