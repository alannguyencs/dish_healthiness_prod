 ...
 *
 * Displays the nutritional analysis results after user confirmation of
 * the identification phase. Stage 8: Manual Edit toggle flips the card
 * into an edit form. Stage 10 adds a parallel "AI Assistant Edit" button
 * that expands an inline textarea; Submit POSTs the hint to the backend
 * which revises the payload via Gemini 2.5 Pro and commits directly (no
 * preview step).
 */
const NutritionResults = ({
  ...
  if (!nutritionData) {
    return (
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
        <p className="text-blue-800">Nutrition analysis in progress...</p>
      </div>
    );
  }

  const activeData = nutritionCorrected || nutritionData;
  const isCorrected = !!nutritionCorrected;
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6 space-y-6">
        ...
        <div>
          ...
          {isCorrected && (
            <span
              ...
              data-testid="nutrition-corrected-badge"
            >
              Corrected by you
            </span>
          )}
        ...
        <div className="flex items-center gap-2">
          {onEditSave && (
            <button
              ...
              data-testid="nutrition-edit-toggle"
            >
          ...
          {onAiAssistSubmit && (
            <button
              ...
              data-testid="nutrition-ai-assistant-toggle"
            >
      ...
      {(nutritionData.model ||
        nutritionData.price_usd ||
        nutritionData.analysis_time) && (
          ...
          <div className="flex items-center gap-6">
            {nutritionData.model && (
              <div className="flex items-center gap-1.5">
                <span>🤖</span>
                <span>Model:</span>
                <span className="font-medium">{nutritionData.model}</span>
              </div>
            )}
            {nutritionData.price_usd && (
              <div className="flex items-center gap-1.5">
                ...
                <span className="font-medium">
                  ${nutritionData.price_usd.toFixed(4)}
                </span>
              </div>
            )}
            {nutritionData.analysis_time && (
