...
Every macro number on the Nutritional Analysis results now ships with a one-line rationale — which database source (if any) drove the calories, which prior dish supported the protein estimate, or "LLM-only" when no external evidence cleared the confidence gates. Weak or absent evidence falls back explicitly to the AI's own reasoning, flagged as "LLM-only" in the attribution so nothing is silently invented. The "Why these numbers?" expandable panel on the review view surfaces all seven rationales.

### Reviewing and correcting the analysis

Below the Nutritional Analysis headline numbers, the user sees three read-only reference panels: an expandable "Why these numbers?" rationale block, a chip row of the top database matches the AI considered (with confidence badges), and a card list of the user's own prior similar dishes (with thumbnails and nutrients).

When the AI gets a number wrong the user has two parallel correction paths on the Nutritional Analysis card. **Manual Edit** flips every editable field into an input for direct numeric correction. **AI Assistant Edit** expands an inline textarea where the user types a natural-language hint; submitting it asks Gemini 2.5 Pro to revise the numbers against the hint + the original image, and the revised payload replaces the card directly (no Accept/Cancel preview). Both paths persist the result in two places: onto this meal's record (so the user sees their corrected numbers from now on) and into the user's personalization history (so future uploads of similar dishes start with the user's verified nutrients, not the AI's original guess). The original AI numbers are kept as audit under the hood — they never disappear, they're just superseded in the UI by the user's override. See [User Customization](./user_customization.md) for the full correction UX.
