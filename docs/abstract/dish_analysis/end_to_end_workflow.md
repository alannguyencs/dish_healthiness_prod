...
```
  ...
  │                                                                       │      │  • Backend loads result_gemini.step2_data (current baseline).         │
  │                                                                       │      │  • Calls Gemini 2.5 Pro: "revise baseline given user context" —       │
  │                                                                       │      │    same JSON shape, change only fields the hint justifies, explain    │
  │                                                                       │      │    each change in healthiness_score_rationale + reasoning_*.          │
  ...
  │       Button B) } while preserving the original step2_data for audit; │
  │       step = 2                                                        │
  │  • UPDATE personalized_food_descriptions  (row inserted in 1.1.1c)    │
  │       SET corrected_step2_data = <final payload>                      │
  │       WHERE query_id = <this query_id>                                │
...
7. **User corrections never replay Phase 1 or the retrievals.** Two correction paths exist on the Item view — **Manual Edit** writes the user's typed values straight through with no LLM call, while **AI Assistant Edit** re-runs a short Phase 2.3-lite Gemini call that revises the current `step2_data` against a free-text user hint and commits the revised payload directly (no preview / Accept-Cancel step). Both land in the same `step2_corrected` + `corrected_step2_data` shape and leave Phase 1 and the 2.1 / 2.2 retrieval artifacts untouched.

## Scope

- **Included:** every phase that fires between upload and the rendered Step 2 card (Phase 1.1.1, 1.1.2, 1.2, 2.1, 2.2, 2.3, 2.4), named with the module that owns it and the JSONB key it writes.
...
- [x] Every JSONB key named in the diagram (`reference_image`, `step1_data`, `step1_confirmed`, `nutrition_db_matches`, `personalized_matches`, `step2_data`, `step2_corrected`) is produced by the running pipeline.
