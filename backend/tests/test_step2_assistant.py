"""
Unit tests for `backend/src/service/llm/step2_assistant.py` (Stage 10).
...
from src.service.llm import step2_assistant
...
def test_trim_drops_metadata_and_audit_fields():
    ...
    trimmed = step2_assistant._trim_baseline_for_prompt(baseline)
    # Kept
...
def test_render_substitutes_baseline_and_hint(tmp_path, monkeypatch):
    ...
    rendered = step2_assistant._render_assistant_prompt(baseline, "smaller portion")
...
async def test_revise_uses_step2_corrected_when_present(monkeypatch, tmp_image):
    ...
    monkeypatch.setattr(step2_assistant, "analyze_step2_nutritional_analysis_async", fake_analyzer)
    monkeypatch.setattr(step2_assistant, "IMAGE_DIR", tmp_image.parent)
    monkeypatch.setattr(
        step2_assistant,
        "get_dish_image_query_by_id",
        lambda _id: _record_with(
            {
                "step2_data": {"calories_kcal": 600, "dish_name": "Ayam"},
                "step2_corrected": {"calories_kcal": 9999, "dish_name": "Ayam"},
            },
            tmp_image.name,
        ),
    )

    result = await step2_assistant.revise_step2_with_hint(1, "fix this")
...
async def test_revise_falls_back_to_step2_data_when_no_corrected(monkeypatch, tmp_image):
    ...
    result = await step2_assistant.revise_step2_with_hint(1, "use baseline")
...
async def test_revise_raises_when_image_missing(monkeypatch, tmp_path):
    monkeypatch.setattr(step2_assistant, "IMAGE_DIR", tmp_path)
    monkeypatch.setattr(
        step2_assistant,
        "get_dish_image_query_by_id",
        lambda _id: _record_with(
            {"step2_data": {"calories_kcal": 500}},
            "not_on_disk.jpg",
        ),
    )
    ...
    with pytest.raises(FileNotFoundError):
        await step2_assistant.revise_step2_with_hint(1, "hint")
...
async def test_revise_raises_when_no_baseline(monkeypatch, tmp_image):
    ...
    monkeypatch.setattr(
        step2_assistant,
        "get_dish_image_query_by_id",
        lambda _id: _record_with({"step": 1}, tmp_image.name),
    )
...
async def test_revise_raises_when_record_missing(monkeypatch):
    monkeypatch.setattr(step2_assistant, "get_dish_image_query_by_id", lambda _id: None)
    with pytest.raises(ValueError):
        await step2_assistant.revise_step2_with_hint(999, "hint")
