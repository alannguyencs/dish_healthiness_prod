"""
Stage 10 — Phase 2.4 AI Assistant Edit revision service.

Loads the current effective Step 2 payload from `result_gemini` (prefers
`step2_corrected` when present, falls back to `step2_data`), builds a
revision prompt from `backend/resources/step2_assistant_correction.md`
...
`Step2NutritionalAnalysis` payload.

Persistence is the endpoint's job — this service only produces the
revised numbers. Mirrors the Stage 7 Gemini wrapper pattern so retries,
...
def _render_assistant_prompt(trimmed_baseline: Dict[str, Any], user_hint: str) -> str:
    prompt_path = RESOURCE_DIR / "step2_assistant_correction.md"
...
async def revise_step2_with_hint(record_id: int, user_hint: str) -> Dict[str, Any]:
    """
    Produce a revised `Step2NutritionalAnalysis` payload for the given
    record, driven by the user's free-text hint.

    Baseline selection: current effective payload — `step2_corrected` if
    present, else `step2_data`. Stacked edits are supported by design.
    ...
    baseline = result_gemini.get("step2_corrected") or result_gemini.get("step2_data")
    if not baseline:
        raise ValueError(
            f"Record {record_id} has no step2_data/step2_corrected baseline to revise"
        )
    ...
    logger.info(
        "Stage 10: revising record_id=%s (baseline source=%s, hint_len=%s)",
        record_id,
        "step2_corrected" if result_gemini.get("step2_corrected") else "step2_data",
        len(user_hint),
    )

    return await analyze_step2_nutritional_analysis_async(
