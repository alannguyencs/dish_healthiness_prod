...
(`item_identification_tasks.py` ↔ `item_tasks.py`).
"""
...
from src.service.llm.identification_analyzer import analyze_component_identification_async
from src.service.llm.prompts import get_component_identification_prompt
...
def _resolve_reference_inputs(
    ...
    Returns (None, None) on any of the degrade paths:
      ...
      - reference.prior_identification_data is None (Option B per 2026-04-18 decision)
      - reference.prior_identification_data is an empty dict (defensive)
    """
    if not reference:
        return None, None
    ...
    prior = reference.get("prior_identification_data")
    ...
    # either prior_identification_data, or a user-verified confirmed_* value.
...
async def analyze_image_background(query_id: int, file_path: str, retry_count: int = 0) -> None:
    ...
    if user_id is not None and not is_retry_short_circuit:
        ...
        # output. Seed with the same `{"phase": 0, "identification_data": None}`
        # defaults persist_phase_error uses so the frontend's
        # step-branching logic stays consistent on a subsequent failure.
        pre_blob = (
            (record_pre.result_gemini or {"phase": 0, "identification_data": None}).copy()
            if record_pre
            else {"phase": 0, "identification_data": None}
        )
    ...
    try:
        ...
        identification_prompt = get_component_identification_prompt(reference=effective_reference)
        identification_result = await analyze_component_identification_async(
            image_path=file_path,
            analysis_prompt=identification_prompt,
        ...
        base.update(
            {
                "phase": 1,
                "identification_data": identification_result,
                "nutrition_data": base.get("nutrition_data"),
                "identification_confirmed": base.get("identification_confirmed", False),
                "iterations": [
                    {
                        "iteration_number": 1,
        ...
        base.pop("identification_error", None)
        ...
        persist_phase_error(query_id, exc, retry_count, "identification_error")
